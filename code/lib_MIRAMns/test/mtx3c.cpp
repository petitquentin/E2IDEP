...
int main(int argc, char *argv[])
{
    ...
    // Params
    bool debug = false;
    std::string pathInput = "../data/mtx_3c/test.mtx";
    ...
    std::vector<unsigned long int> col;
    std::vector<double> val;
    unsigned long int nb_rows, nb_cols, nnz;
    read_mtx(pathInput, row, col, val, nb_rows, nb_cols, nnz);
    if (debug)
    {
    ...
    std::cout << "Size of col : " << col.size() << std::endl;
    ...
    if (debug)
    {
        ...
        {
            std::cout << " - " << i << ": " << row[i] << " " << col[i] << " val " << val[i] << std::endl;
        }
    }
    double *v = NULL;
    mtx_to_dense(&v, row, col, val, nb_rows, nb_cols, MPI_COMM_WORLD);
