...
int main(int argc, char *argv[])
{
    ...
    // Params
    bool debug = false;
    std::string pathInput = "../data/mtx_2c/test.mtx";
    ...
    read_mtx(pathInput, row, col, nb_rows, nb_cols, nnz);
    if (debug)
    {
    ...
    if (debug)
    {
        ...
        {
            std::cout << " - " << i << ": " << row[i] << " " << col[i] << std::endl;
        }
    }
    bool *v;
    mtx_to_dense(&v, row, col, nb_rows, nb_cols, MPI_COMM_WORLD);
