...
int main(int argc, char *argv[])
{
    ...
    std::string pathOutput = "../../output/eigenvectors.mtx";
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        if (strcasecmp(argv[i], "-help") == 0 || strcasecmp(argv[i], "-h") == 0)
        {
            ...
            std::cout << " -o or -output : the path to the output mtx file " << std::endl;
    ...
    if (lmax < k_size)
    {
        if (rank == 0)
        {
            std::cout << "Invalid lmax (l parameter): change by k (previous value : " << lmax << ", new : " << 2 * k_size << ")" << std::endl;
        }
        lmax = k_size;
    }
    if (debug && rank == 0)
    ...
    Eigen::SparseMatrix disA = read_mtx_sparse(pathInput, startRow, 0, v_size[rank], nb_cols);
    ...
    for (int i = 0; i < v_size[rank]; i++)
    {
        ...
        {
            norm_A += disA.coeff(i, j) * disA.coeff(i, j);
        }
    }
    ...
    // Subspace autom
    int subspace_size = abs(lmax - (2 * k_size)) + 1;
    Eigen::VectorXd subspaces_size(subspace_size);
    if(lmax < (2 * k_size)){
        for (int i = 0; i < subspace_size; i++)
        {
            subspaces_size(i) = lmax + i;
            std::cout << (subspaces_size(i)) << std::endl;
        }
    ...
    if (rank == 0)
    {
        ...
        if (debug)
        {
            ...
            for (int i = 0; i < k_size; i++)
            {
                ...
                std::cout << " (" << i << ") - [" << U_desired.coeff(0, i);
                for (int j = 1; j < disA.cols(); j++)
                {
                    std::cout << ", " << U_desired.coeff(j, i);
                }
                std::cout << "]" << std::endl;
            }
        }
    ...
    // Save eigenvectors in MTX file
    save_mtx_dense(U_desired, pathOutput);
