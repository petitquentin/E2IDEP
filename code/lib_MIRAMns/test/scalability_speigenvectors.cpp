...
int main(int argc, char *argv[])
{
    ...
    Eigen::SparseMatrix<double> disA = Eigen::SparseMatrix<double>(v_size[rank], nb_cols);
    // disA = (read_mtx(pathInput)).block(startRow, 0, v_size[rank], nb_cols);
    disA = read_mtx_sparse(pathInput, startRow, 0, v_size[rank], nb_cols);
    ...
    distributed_multi_iram_nolog(disA, numVectors, k_size, tolerance, max_it, desired_eigenvalues_Hmm, U_desired, subspaces_size, norm_A, v_size, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    end = std::chrono::high_resolution_clock::now();
    // output
    if (rank == 0)
    {
        std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / std::pow(10, 9) << std::endl;
    }
